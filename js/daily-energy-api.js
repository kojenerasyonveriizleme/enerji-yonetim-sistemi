...
const DailyEnergyAPI = {
    ...
    listDailyEnergy: function(limit = 100, offset = 0) {
        return new Promise((resolve, reject) => {
            ...
            console.log('📤 DailyEnergyAPI list isteği:', {action: 'list', module: 'daily-energy', limit, offset});
            ...
            .then(data => {
                console.log('📥 DailyEnergyAPI list yanıtı:', data);
                console.log('📥 data.success:', data.success);
                console.log('📥 data.data:', data.data);
                console.log('📥 data.data.data:', data.data?.data);
                
                if (data.success) {
                    // Sadece data.data.data formatını kabul et
                    if (data.data && data.data.data && Array.isArray(data.data.data)) {
                        console.log('📋 Format: data.data.data');
                        resolve(data.data.data);
                    } else {
                        console.log('❌ Beklenmeyen format:', data);
                        console.log('❌ data.data:', data.data);
                        console.log('❌ data.data.data:', data.data?.data);
                        resolve([]);
                    }
