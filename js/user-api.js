...
const UserAPI = {
    ...
    init: function() {
        ...
        // Otomatik senkronizasyon (localStorage'daki kullanıcıları Google Sheets'e aktar)
    ...
    syncLocalUsersToCloud: async function() {
        try {
            ...
            if (result.success) {
                ...
                // Kullanıcı yönetimi sayfasında senkronizasyon durumunu güncelle
    ...
    updateSyncStatus: function(status) {
        ...
        if (syncStatusElement) {
            ...
            switch(status) {
                case 'success':
                    syncStatusElement.textContent = timeString + ' ✅';
                    syncStatusElement.style.color = '#047857';
                    break;
                case 'error':
                    syncStatusElement.textContent = timeString + ' ❌';
                    syncStatusElement.style.color = '#b91c1c';
                    break;
                case 'syncing':
                    syncStatusElement.textContent = '...';
                    syncStatusElement.style.color = '#ca8a04';
                    break;
                default:
                    syncStatusElement.textContent = '-';
                    syncStatusElement.style.color = '#64748b';
            }
        }
    },
