...
const GoogleSheetsAPI = {
    ...
    saveData: async function(module, data, action = 'save') {
        try {
            ...
            Object.keys(data).forEach(key => {
                // ✅ allData gibi objeleri JSON.stringify yap
            ...
            if (result.success) {
                ...
                console.error(`${module} kayıt hatası:`, result.error);
    ...
    /**
     * Kayıtları getir - Toplu işlem için
     * @param {string} module - Modül adı
     * @param {Object} filters - Filtreler
     */
    getRecords: async function(module, filters = {}) {
        return await this.getData(module, filters);
    },
    ...
    // ==================== KOJEN MOTOR ÖZEL FONKSİYONLARI ====================

    /**
     * Kojen motor kayıt kontrolü
     * @param {string} tarih - Tarih
     * @param {string} vardiya - Vardiya tipi
     */
    checkKojenMotorRecord: async function(tarih, vardiya) {
        try {
            const url = CONFIG.GOOGLE_SHEETS_WEB_APP_URLS.kojen_motor;
            if (!url) {
                console.error('Kojen Motor URL bulunamadı');
                return { success: false, error: 'Modül URL bulunamadı' };
            }

            const formData = new FormData();
            formData.append('action', 'check');
            formData.append('module', 'kojen_motor');
            formData.append('tarih', tarih);
            formData.append('vardiya', vardiya);
            ...
            const result = await response.json();
            console.log('Kojen Motor kayıt kontrolü:', result);
            
            return result;

        } catch (error) {
            console.error('Kojen Motor kayıt kontrolü hatası:', error);
            return { success: false, error: error.message };
        }
    },
     ...
     * @param {string} tarih - Tarih
     * @param {string} vardiya - Vardiya tipi
     * @param {Object} allData - Tüm saat bazlı veriler
     */
    saveKojenMotorRecord: async function(tarih, vardiya, allData) {
        try {
            const url = CONFIG.GOOGLE_SHEETS_WEB_APP_URLS.kojen_motor;
            if (!url) {
                console.error('Kojen Motor URL bulunamadı');
            ...
            const result = await response.json();
            console.log('Kojen Motor kayıt:', result);
            
            return result;

        } catch (error) {
            console.error('Kojen Motor kayıt hatası:', error);
            return { success: false, error: error.message };
        }
    },

    /**
     * Kojen motor güncelle
     * @param {string} id - Kayıt ID
     * @param {Object} allData - Tüm saat bazlı veriler
     */
    updateKojenMotorRecord: async function(id, allData) {
        try {
            const url = CONFIG.GOOGLE_SHEETS_WEB_APP_URLS.kojen_motor;
            if (!url) {
                console.error('Kojen Motor URL bulunamadı');
            ...
            const result = await response.json();
            console.log('Kojen Motor güncelleme:', result);
            
            return result;

        } catch (error) {
            console.error('Kojen Motor güncelleme hatası:', error);
            return { success: false, error: error.message };
        }
    },
     ...
     */
    checkRecord: async function(module, data) {
        try {
            const url = CONFIG.GOOGLE_SHEETS_WEB_APP_URLS[module];
            if (!url) {
                console.error(`Google Sheets URL bulunamadı: ${module}`);
            ...
            const result = await response.json();
            console.log('Kayıt kontrolü:', result);
